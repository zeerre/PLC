...
2. ISBN码。（NOIP2008普及组复赛，isbn，1s，64MB）

* 【问题描述】

每一本正式出版的图书都有一个ISBN码与之对应。ISBN码包括9位数字、1位识别码和3位分隔符，其规定格式如 $x-xxx-xxxxx-x$ ，其中符号“-”是分隔符（键盘上的减号），最后一位是识别码。例如`0-670-82162-4`就是一个标准的ISBN码。ISBN码的首位数字表示书籍的出版语言，例如0代表英语；第一个分隔符“-”之后的3位数字代表出版社，例如670代表维京出版社；第二个分隔符之后的五位数字代表该书在出版社的编号；最后一位为识别码。

识别码的计算方法：首位数字乘以1,加上次位数字乘以2, $\ldots$ 以此类推，用所得的结果 `mod 11`（% 11），所得到的玉树即为识别码，如果余数为10,则是别码为大写字母X。例如ISBN码`0-670-82162-4`中的识别码4是这样得到的：对067082162这9个数字，从左到右，分别乘以1,2,...,9，再求和，即 $0\times 1+6\times 2+ \ldots +2\times 9=158$ ，然后取`158 mod 11`（158%11）的结果4作为识别码。

本题的任务是编写程序判断输入的ISBN码中识别码是否正确，如果正确，则仅输出“Right”；如果错误，则输出正确的ISBN码。

* 【输入格式】

一行一个字符序列，表示一本书的ISBN码（保证输入符合ISBN码的格式要求）。

* 【输出格式】

共一行，加入输入的ISBN码的识别码正确，那么输出“Right”；否则按照规定的格式，输出正确的ISBN码（包括“-”分隔符）。
...
3. 麦铅笔（NOIP2016普及组复赛，pencil，1s，64MB）

* 【问题描述】

林老师需要去商店买n支铅笔作为小朋友们参加NOIP的礼物。他发现商店一共有3种包装的铅笔，不同包装内的铅笔数量有可能不同，价格也有可能不同。为了公平起见，他决定只买同一种包装的铅笔。商店不允许将铅笔的包装拆开，因此林老师可能需要购买超过n支铅笔才够给小朋友们发礼物。现在林老师想知道，在商店每种包装的数量都足够的情况下，要买够至少n支铅笔最少需要花费多少钱。

* 【输入格式】

第一行一个正整数n，表示需要铅笔的数量。接下来三行，每行用两个正整数描述一种包装的铅笔：其中第一个整数表示这种包装内铅笔数量；第二个整数表示这种包装的价格。
...
铅笔的三种包装分别是：两只装，价格2；50支装，价格30；30只装，价格27。林老师需要购买至少57支铅笔。如果他选择购买第一种包装，那么他需要购买29份，共计 $2\times 29=58$ 支，需要花费 $2\times 29 =58$ 元。实际上,林老师会选择购买第三种包装，这样需要买2份。虽然最后买到的铅笔数量更多了，为 $30\times 2=60$ 支，但花费减少为 $27\times 2=54$ 元，比第一种少。对于第二种包装，虽然每支铅笔的价格最低，但要够发必须买两份，实际的花费达到了 $30\times 2=60$ 元，因此林老师也不会选择。所以最后输出的答案是54。

* 【子任务】

子任务会给出部分测试数据的特点。如果解决题目中遇到了困难，可以尝试只解决一部分测试数据。每个测试点的数据规模及特点见下表。在表中“整倍数”的意义为：若“K”，表示对应数据作需要的铅笔数量n一定是每种包装铅笔数量的整倍数（这意味着一定可以不用多买铅笔）。

|测试点|整倍数|其他特点|
|-|-|-|
|1,2,3,4| $\surd$ |三种包装内的铅笔数量都是相同的|
|5,6,7,8| $\times$ ||
|9,10,11,12| $\surd$ |后两种包装的铅笔数量是相同的|
|13,14,15,16| $\times$ ||
|17,18| $\surd$ | 没有特殊性质|
|19,20| $\times$ ||
...
1. 理解循环结构的含义
2. 熟练掌握while语句的格式和功能
3. 初步应用while语句解决一些实际问题

### 知识点

在程序设计中，经常需要反复执行某一条语句或语句块，这种结构称为“循环结构”或者“重复结构”。其实，计算机最擅长的就是重复作一件事情，所以循环结构程序设计中应用非常广泛。在C++中，循环结构有三种实现语句：while语句、do-while语句、和for语句。
...
```
其含义为：先计算表达式（一般为循环条件）的值，当表达式的值为真（循环条件成立）时，去执行一次循环体。和if语句不同的是，执行完一次循环体后，while语句又回到开始处，继续计算和判断表达式的真假，决定是否再次执行循环体。也就是“当表达式成立时，不断重复执行循环体”，所以又称为“当型循环”，如下图：
...
以下代码段就是可以执行10次循环体，每次输出一个‘@’，并且i自加1。所以，整个代码的作用就是输出10个‘@’。一般把i称为循环控制变量。
...
如果把 `i++` 修改为`i+=2;`，运行发现，程序输出5个”@“。如果再把`i<=10`修改成`i<1;`，运行发现，程序没有任何输出，因为第一次循环条件就不成立，循环体一次也没执行。如果不小心将循环体重的语句`i++`漏掉了，会怎么样呢？通过分析和上级测试发现，程序会一直执行`printf("@");`语句，永远也不会结束。这种情况称为”死循环“。所以在while语句中，一定要注意”循环条件表达式“的书写，以及循环体中”循环控制变量”的更新，从而避免死循环。同时循环条件表达式和循环控制变量的变化，也决定了循环体被执行的次数。

> 2. while 语句的应用举例

* 例1 求 $1+2+3+\cdots+100$ 的值。

* 【问题分析】

设sum存放答案，初始化为0.再设i为循环控制变量，初始化为1.当循环条件（i<=100）成立时，反复执行以下循环体：
...
一般采用列表观察、找规律的方法阅读和理解程序。循环条件为`i>0`，用到i，x和y这3个变量，如下表：

|i的取值|循环条件|循环体执行情况|
|-|-|-|
|100|非0|i更新为99,x赋值为3,y的值不变还是0|
|99|非0|i更新为98,x赋值为2,y的值不变还是0|
|98|非0|i更新为97,x赋值为1,y的值变为1|
|97|非0|i更新为96,x赋值为0,y的值不变还是1|
|96|非0|i更新为95,x赋值为7,y的值不变还是1|
| $\vdots$ | $\vdots$ | $\vdots$ |
|1|非0|i更新为0,x赋值为0,y的值不变还是13|

通过找规律发现，每次i是`8*x+1`时，y的值会加1,也就是 $97、89、81、\cdots 、9、、1$ ，一共99/8+1=13.所以程序最后输出y的值为13。

* 例3 统计正数。

* 【问题描述】

输入若干整数，以0结尾，统计其中有多少个正整数。
...
对于本题来说，首先要输入一个整数，当他不为0时，需要反复执行：

（1）判断该书是否为正数，如果是，则答案累加1；
...
消化在去年转了一大笔钱。他想把这些钱用于投资，并对自己能得到多少收益感到好奇。已知投资的年利率为 $r(0\sim 20 \text{之间的整数})$ ，小华现有m元钱。他想知道投资多少年后，他的前将超过y元。

* 【输入格式】

一行三个整数 $r , m ,y$ 相邻两个整数之间用一个空格隔开。m为 $100\sim 1000000$ 之间的整数。

* 【输出格式】

一行一个整数，即要投资的年数。保证答案的范围在 $1\sim 400$ 之间。
...
小华手里有m元钱，目标时超过y元钱，k表示答案（初始化0）。所以当条件 $m\leqslant y$ 成立时，要反复执行。
...
1. 阅读程序，写出程序的运行结果。（NOIP2016普及组初赛）

```
#include<iostream>
using namespace std;
int main(){
    int max,min,sum,cout=0;
    int tmp;
    cin>>tmp;
    if(tmp==0) return 0;
    max=min=sum=tmp;
...
2. 斐波那契数列。（fib，1s，128MB）

* 【问题描述】

斐波纳契数列时指这样的数列：第一个数和第二个数都是1,接下来每个数都等于前面2个数之和。编程输入一个正整数k，输出斐波纳契数列第k个数。
...
3. 人口增长问题。（population，1s，128MB）

* 【问题描述】

假设目前的世界人口有 x 亿，按照每年 0.1% 的增长速度，n年后酱油多少人？

* 【输入格式】

一行两个正整数x和n，之间有一个空格。其中， $1\leqslant x \leqslant 100, 1\leqslant n\leqslant 100$ 。

* 【输出格式】

一行一个数，表示答案。以亿为单位，保留到小数点后4位。
...
4. 球的高度。（height，1s，128MB）

* 【问题描述】

一个皮球从某一高度落下（整数，单位：米），每次落地后反跳回原来高度的一半再落下。编程计算在第10次落地时，皮球一共经过了多少米？第10次可以反弹到多高？

* 【输入格式】

一行一个整数h，int范围内，表示球的初始高度。

* 【输出格式】

第一行：皮球第10次落地时，一共经过的米数。

第二行：第10次弹跳的高度。

注意：结果可能是实数，建议用`double`类型保存，直接用`cout`输出即可。
...
1. 熟练掌握`do-while`语句的格式和功能。
2. 体会 `do-while` 语句与 `while`语句的区别。
3. 熟练运用 `do-while` 语句解决一些实际问题。

### 知识点

`do-while` 语句是C++中用于解决至少执行一次重复操作（循环体）的循环语句。
...
其含义为：先执行一次循环体，然后判断表达式是否成立，如果成立，则返回继续执行循环体，直到表达式不成立，才退出循环。一般称之为“直到型循环”，如下图：
...
`do-while`语句与`while`语句都可以实现循环结构，区别是：`do-while`语句先执行循环体，再判断表达式。所以，至少执行一次循环体。

> do-while 语句的应用举例

* 例1 用 do-while 语句实现：输出一行10个“@”。
...
用 do-while 语句实现：输入若干整数，以0结尾，统计其中有多少个正整数。
...
求一个正整数的各位数字之和，需要“分解”出它的每一位数字，每次进行累加。定义累加器a，初始化为0。对于正整数n，分解的过程就是从低位到高为，不断求余累加（s+=n%10）和整除(n/=10),直到n=0 。
...
所谓角谷猜想，是指对于任意一个正整数，如果是奇数，则乘3加1；如果是偶数，则除以2。得到的结果在按照上述规则重复处理，最终总能够得到1。请编程验证角谷猜想。

* 【输入格式】

一行一个正整数N， $1\leqslant N\leqslant 2000000$ 。

* 【输出格式】

若干行，每行表示一步运算，最后一行输出 “End”，具体格式参见样例输出。
...
给定一个整数，请将该数各个位上数字反转得到一个新数。新数也应满足整数的常见形式，即除非给定的原数为0,否则反转后得到的新书的最高位数字不应为零（参见例2）。

* 【输入格式】

一行一个整数N， $-10^9\leqslant N \leqslant 10^9$ 。
...
1. 熟练掌握for语句的格式、功能和应用场合。
2. 学会用for语句改写while和do-while语句。
3. 熟练应用for语句解决一些实际问题。

### 知识点

在实际应用中，如果重复执行的操作（循环体）此书是固定的、已知的，则一般使用for语句。for语句格式如下：
...
给出一个班级里面每名学生的年龄（正整数），求所有学生的平均年龄，保留小数点后2位。

* 【输入格式】

一行一个正整数n表示学生的人数 ， $1\leqslant n\leqslant 100$ 。
...
给定k个正整数，每个数都是大于或等于1,且小于或等于10。编程统计其中1、5和10出现的个数。

* 【输入格式】

第一行一个正整数k。第二行包含k个正整数，每两个正整数之间用一个空格隔开。

* 【输出格式】

第一行位1出现的次数，第二行为5出现的次数，第三行为10出现的次数。
...
1. 若有如下程序段（设s、a、b、c均已经定义为整形变量，且a、c均已赋值，c大于0）：
...
3. 最高分。（maxscore，1s，128MB）

* 【问题描述】

林老师讲授的《计算机概论》这门课程期中考试刚刚结束，他想知道考试中的最高分。因为人数比较多，他觉得这件事情交给计算机来做比较方便。请帮助林老师解决这个问题。

* 【输入格式】

第一行为正整数n $(1\leqslant n\leqslant 101)$ ，表示参加这次考试的人数。

第二行是这n个学生的成绩，相邻两个数之间用一个空格隔开，所有成绩均为 $0\sim 100$ 之间的整数。
...
4. 奖牌计数。（medal，1s，128MB）

* 【问题描述】

2020年奥运会，A国的运动员参与了n天的决赛项目 $(1\leqslant n\leqslant 17)$ 。现在要统计以下 A国所获得的金、银、铜牌数目及总奖牌数。

* 【输入格式】

第一行时A国参与决赛项目的天数n；

其后n行，每一行是该国某一天获得的金、银、铜牌数目，以一个空格隔开。

* 【输出格式】

一行4个整数，为A国所获得的金、银、铜牌总数及中奖牌数，两个整数之间用一个空格隔开。
...
5. 奇数求和。（oddsum，1s，128MB）

* 【问题描述】

计算非负整数 $m\sim n$ （包括m和n）之间的所有奇数的和， $0\leqslant m\leqslant n\leqslant 300$ 。
...
在第三单元中，已经介绍了分支结构的嵌套。循环结构与之类似，也可以在循环语句的循环体里出现另一个循环语句，不管是while语句、do-while语句还是for语句。这样的循环结构称为“循环嵌套”。
...
一行两个正整数n和m，中间用一个空格隔开， $1\leqslant n,m\leqslant 100$ 。
...
要输出n行，所以可以编写一个外层循环，穷举型。然后再循环里，还要输出m个“*”，所以再嵌套一个内层循环，穷举美一行的“*”个数。
...
输入一个正整数n，输出n行的数字三角形。其中，第1行为数字1,第2行为数字23,第3行为数字456,第4行为数字7890,第5行为数字12345, $\ldots$

* 【输入格式】

一行一个正整数n， $1\leqslant n\leqslant 100$ 。
...
一个正整数，如果他能被7整除，或者它的某一位上的数字为7,则称其为“与7相关”的数。现在请编程求出所有小于或等于n的“与7无关”的正整数个数。
...
1. 球最大跨度值。（maxspan，1s，128MB）

* 【问题描述】

给定一个长度为n的非负整数序列，请计算序列的“最大跨度值”。最大跨度值是指用序列中的最大数减去最小数。

* 【输入格式】

第一行为序列的个数n， $1\leqslant n\leqslant 1000$ ；

第二行为n个不超过1000的非负整数，之间用一个空格隔开。
...
2. 求含k个3的数。（threek，1s，128MB）

* 【问题描述】

输入3个正整数m、n、k，其中： $ 1\lt m\leqslant n\lt 100000 , 1\lt k\lt 5$ ,判断m和n之间有多少个数能被19整除（含m和n），且恰好含有k个3。
...
3. 求阶乘的和。（sumfact，1s，128MB）

* 【问题描述】

给定正整数n，求不大于n的正整数的阶乘的和，即求 $1!+2!+3!+\ldots+n! ,$ 其中， $n!=1\times 2\times 3\times \cdots \times n$ 。

* 【输入格式】

一行一个正整数n， $1\lt n\lt 12$ 。
...
在编写程序的过程中，错误是在所难免的。一般说来，程序代码越长、算法和数据结构越复杂，出错的概率越高。因此调试就成了极其重要的一个环节。学会程序的调试与跟踪，可以快速、准确的发现并改正错误。

> 1.静态差错

程序写好后，首先要从头至尾通读程序，检查是否存在下面几个问题：语句的顺序、思路、逻辑是否有误；语法是否有误，如关键字是否录入错误、变脸是否没有定义就使用、变量的大小写问题、雍错标点符号等。其他常见的错误还有以下几个方面：
...
这是C++中条是程序的常用方法。比如，向查看以下程序中，当 `x%i==0`成立时变量x和i的值，则可以在if语句例输出这两个变量的值。
...
int main(){
    ...
    for(int i=2;i<=sqrt(x);i++){
    	    if(x%i==0){
		cout<<x<<" "<<i<<endl; //用于测试输出x和i的值；测试完毕后应删除或注释掉。
		cout<<"not ";
		break;
	    }
    }
...
1. 编程计算圆周率 $\pi$ 。已知 $\pi /4=1-1/3+1/5-1/7+\cdots$ ，直到某一项的绝对值小于 $10^6$ 为止。

2. 数的累加。（sum，1s，128MB）

将正整数m和n之间（包括m和n）能被17整除的时累加出来。

* 【输入格式】

一行两个正整数m和n，以一个空格隔开， $0\lt m\lt n\lt 1000$ 。
...
3. 求完全数。（totally，1s，128MB）

一个正整数如果恰好等于他的所有因子之和，这个数就称为“完全数”。例如，6的因子为1、2、3,而6=1+2+3,因此6是“完全数”。编程找出m和n之间（包括m和n）的所有完全数。

* 【输入格式】

一行两个正整数m和n， $0\lt m\lt n\leqslant 10000$ 。

* 【输出格式】

如干行，每行一个正整数，表示所有的完全数，按从小到大的顺序输出。
...
1. 总结与反思建立循环结构的知识体系。
2. 实践巩固，熟练应用循环结构解决一些实际问题。

### 知识讲解

> 例1 金币问题。（NOIP2015普及组复赛，coin，1s，64MB）

* 【问题描述】

国王将金币作为工资，发给忠诚的骑士。第一天骑士收到一枚金币；之后两天（第二天和第三天），每天收到两枚金币；之后三天（第四、五、六天），每天受到三枚金币；之后四天（第七、八、九、十天），每天收到四枚金币， $\cdots\cdots$ 这种工资发放模式会一直这样延续下去。当连续N天每天收到N枚金币后，骑士会在之后的连续N+1天里，每天收到N+1枚金币。请编程计算在前K天里， $1\leqslant K\leqslant 10000$ ，骑士以共获得了多少枚金币。
...
骑士第一天收到一枚金币；第二天和第三天收到两枚金币；第四、五、六天收到三枚金币。因此以共收到1+2+2+3+3+3=14枚金币。

* 【分析】

设共有N天，当前为第i天，则当 $i\leqslant N$ 时，可以统计之后连续K天所的金币，在统计过程中，如果 $i\gt N$ ，则退出统计。

> 例2 比例简化。（NOIP2014普及组复赛，ratio，1s，64MB）

* 【问题描述】

在社交媒体上，经常会看到针对某一个观点同意与否的民意调查以及结果。例如，对某一观点表示支持的有1489人，反对902人，那么赞同与反对的比例可以简单的记为1489：902。

不过，如果把调查结果就以这种方式呈现出来，大多数人肯定不会满意。因为这个比例的数值太大，难以一眼看出它们的关系。对于上面这个例子，如果把比例记为5：3,虽然与真实结果有一定的误差，但依然能够较为准确地反映调查结果，同时也显得比较直观。

现给出支持人数A，反对人数B，以及一个上限L，请将A比B化简为 A‘ 比 B’，要求在 A‘ 和 B’ 均不大于L 且 A‘ 和 B’ 互质（两个整数的最大公约数是1）的前提下， A‘ / B’ $\geqslant A/B 且 A‘ / B' - A / B$ 的值尽可能小。

* 【输入格式】

一行三个正整数A、B、L，每两个正整数之间用一个空格隔开，分别表示支持人数、反对人数以及上限。其中， $1\leqslant A\leqslant 1000000, 1\leqslant B\leqslant 1000000,1\leqslant L\leqslant 100 , A/B\leqslant L$ 。
