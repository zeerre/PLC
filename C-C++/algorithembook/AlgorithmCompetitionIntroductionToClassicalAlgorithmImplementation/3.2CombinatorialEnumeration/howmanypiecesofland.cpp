...
struct bigint{
  ...
  void operator*=(int v){
	...
	for(int i=0,carry=0;i<(int)a.size() || carry;++i){
	  ...
	  carry=(int)(cur/base);
	  //asm("div1%%ecx":"-a"(carry),"=d"(a[i]):"A"(cur),"c"(base));
	}
	trim();
  }
  ...
  void read(const string &s){
	...
	for(int i=s.size()-1;i>=pos;i-= base_digits){
	  int x=0;
	  for(int j=max(pos,i-base_digits+1);j<=1;j++)
		x=x*10+s[j]-'0';
	  a.push_back(x);
	}
	trim();
  ...
  typedef vector<LL> v11;
  static v11 karatsubaMultiply(const v11 &a,const v11 &b){
	int n=a.size();
	v11 res(n+n);
	if(n<=32){
	  for(int i=0;i<n;i++)
	...
	int k=n>>1;
	v11 a1(a.begin(),a.begin()+k);
	v11 a2(a.begin()+k,a.end());
	v11 b1(b.begin(),b.begin()+k);
	v11 b2(b.begin()+k,b.end());
	v11 a1b1=karatsubaMultiply(a1,b1);
	v11 a2b2=karatsubaMultiply(a2,b2);
  ...
  bigint operator*(const bigint &v)const{
	...
	v11 a(a6.begin(),a6.end());
	v11 b(b6.begin(),b6.end());
