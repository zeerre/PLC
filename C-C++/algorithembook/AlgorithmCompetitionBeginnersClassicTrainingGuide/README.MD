...
* 问题1：数字三角形。如下图所示，有一个有非负整数组成的三角形，第一行只有一个数，除了最下行外，每个树的左下方和右下方各有一个数。从第一行的数开始，每次可以往左下右下走一格，直到走到最下行，把沿途经过的数全部加起来。如何走可使得这个和最大?
...
* 问题3 硬币问题。有n种硬币，面值分别为V<sub>1</sub>,V<sub>2</sub>,...,V<sub>n</sub>，
